...
- Density-discipline weakness signals (no new rubric dimension, no blocking signal): `PHYSICAL CLUE ANCHOR CHECK` requires the before/after change to be statable in one sentence each, anchored to a single fixed marker (→ `specificity`, weakness + revisionAction); `NAMED WITHOUT FUTURE USE` adds an inventory-to-consequence clause for POV inventories of ≥3 named figures (→ `freshness`, weakness + revisionAction); `WITHHELD ACTION VARIETY` mirrors the `WITHHELD_ACTION_VARIETY` validator and asks the judge to count `did/had not + perception/action verb` beats themselves so the signal reaches the revision pass without round-tripping through final audit (→ `freshness`, weakness + revisionAction); `OVER-COMPOSED CLUSTER CHECK` flags the densest ornate cluster identified by opening phrase + scene number (→ `proseQuality`, weakness + revisionAction); `ANTI-COMMITTEE PRINCIPLES` carries the necessity-over-abundance line as a principle paired with the density governors (no per-signal action surface); `DOMINANT JOB DISCIPLINE` reads `ChapterSpec.purpose` directly from the approved-spec JSON in the judge prompt and flags competing material (→ `forwardMotion`, weakness + revisionAction). The revision pass consumes both `weaknesses` and `revisionActions`. Voice-grit downstream does not see judge review and is unchanged by these signals.
