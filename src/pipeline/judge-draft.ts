...
export function buildJudgeInstructions(
  ...
  return [
    ...
    "",
    "CLARITY FLOOR AT DANGER REVEALS (feeds proseQuality).",
    "When a `physicalClueAnchors` entry's change is delivered to the reader inside the chapter — the moment the after-state becomes visible — the prose must contain at least one short plain sentence that restates the change in unornamented terms (e.g., 'The thread had moved.' / 'The water was wrong.' / 'The current had reversed.' / 'The seal had cracked.'). Lyrical compression, metaphor, balanced clauses, and rhetorical doubling are allowed — even celebrated — alongside the plain sentence. The plain sentence does not replace the lyricism; it anchors it. When the change is delivered ONLY through compressed metaphor or extended figurative construction (the prose is beautiful, but the reader must parse the metaphor to know what physically changed), lower proseQuality, name the over-compressed reveal beat in weaknesses by its scene number and the marker that failed to get its plain sentence, and add a concrete revisionAction asking for one blunt fact sentence to be added inside that beat. Bias toward not flagging when the prose already contains an unambiguous physical statement of the change alongside its metaphor, even a four- or five-word one. This is a weakness signal only; never add it to `blockingIssues`.",
    "",
    ...
    "",
    "EXPERT INACTION JUSTIFICATION (feeds characterTruth).",
    "When a POV character whose voice card or active-cast role describes them as a domain expert in a system relevant to this chapter (architect, engineer, doctor, pilot, navigator, captain, scientist, technician, surgeon, mechanic, electrician, etc.) observes a structural anomaly, warning sign, mechanical fault, or safety-threatening event AND takes no observable action AND makes no observable plan to act within the chapter, the prose must make the reason for inaction legible. A reader should be able to state in one short sentence why the expert did not act — for example: a procedural barrier (no one to tell yet), a social cost (would publicly contradict a host or superior), a political or institutional cost (would end a career or a project), a personal entanglement (debt, loyalty, history, leverage being held), a prior commitment (something the character has already promised to do or not do), specific in-room obstruction (the room is mid-toast, the room is full of investors, the next safe moment has not arrived), technical uncertainty (the expert is not yet sure what they saw), or active denial. The reason need not be voiced in dialogue; an interior thought, a glance at the obstacle, a word said and withdrawn, or a remembered prior conversation is enough. When no such reason is legible from the prose — when the silence reads as plot-convenient rather than tragic-because-believable — lower characterTruth, name the silent expert and the specific danger or anomaly they observed in weaknesses, and add a concrete revisionAction asking for one brief in-POV beat that names the reason for delay. Distinguish tragic hesitation from plot-convenient stupidity: the chapter is well-served when the reader can answer 'why didn't they just tell someone?' from material already on the page. Bias toward not flagging when at least one concrete reason is legible, even briefly, even by implication tied to a specific on-page object or person. This is a weakness signal only; never add it to `blockingIssues`.",
    "",
